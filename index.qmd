...
flat_data_long <- gather(flat_data, key="measure", value="value", colnames(flat_data)[!(colnames(flat_data) %in% c("Tag"))])
...
ggplot(flat_data_long, aes(x=Tag, y=value)) + #  %>% group_by(Tag)
  ...
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75), size=0.2, colour="black", fill = NA, scale="width") + # TODO: Find more representative scale type
  facet_wrap(~measure, scales="free_y", ncol=2) +

  # # Écart type / barres d'érreure
  geom_errorbar(data=dfTab, aes(x=Tag, y=mean, ymin=mean-sd, ymax=mean+sd), width=.2, linewidth=.2, colour="red") +
