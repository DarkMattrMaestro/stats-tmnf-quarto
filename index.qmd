...
Features focusing on the car’s rotation at a given moment minimally inform the selected Tag. In fact, these six features, PercentPitch——Third and PercentRoll——Third, are the six worst features in terms of feature importance. It is, therefore, worth re-evaluating the inclusion and implementation of these two features in the case of a future model.


An improved version of the predictive model could be used to suggest Tags to new map authors on TMNF-X. Given that the model can predict but one Tag at a time, only those not knowing any Tag to assign could use it. It would, therefore, be much more useful if the model could perform multi-label classification. In addition, the model in its current form would suggest the wrong Tag to roughly one in twenty authors. The addition of other track details, such as blocks present in the map, would better serve this context and would likely increase exactitude and precision.