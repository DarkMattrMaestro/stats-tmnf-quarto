...
This study’s population consists of maps published on TMNF-X that have only one assigned Tag so as to allow multi-class classification, that do not require game modifications such as TMUnlimiter, and that are of type “Race.” The population size is, thus, approximately 600 000 maps [@TMNFXStats]. While multi-label classification would be preferable as it would allow maps with multiple assigned Tags to be classified, resulting complexities would exceed the scope of this project.

*Table 3.1*

Five Tags, seen in Table 1, are excluded since they either go against the goal of this study, could not reasonably be analysed within given time constraints, or both simultaneously. Eight tags remain: { \textcolor{gray}{Normal, Offroad, Fullspeed, LOL, Tech, SpeedTech, PressForward, Grass} }.


The data collected for this observational study is, first, stratified by each map’s Tag, randomly choosing 1113 maps per stratum. Then, each map’s record — that being the Replay with the lowest finish time — is taken by means of TMNF-X’s API. Only maps with five or more submitted replays are considered. These two, final conditions aim to reduce the number of partial-effort replays entering the sampleset, thus allowing better representation of the map’s true path.

After sampling, the replays are parsed so as to extract meta-data and to calculate other useful information, that is, to collect the dependent variable and the independent and confounding variables. All hierarchical data, such as the position at each time step, is then flattened into a format usable by classification models. See Annex 2 to view the anonymized, collected data.
...
Figure 2 demonstrates the three models’ exactitudes by means of confusion matrices. Note that a perfect model would have no incorrectly assigned Tag and would, thus, form a perfect diagonal from the top left to the bottom right.
...
We determine each feature’s importance on the prediction of a Tag using the SHapley Additive exPlanations [SHAP] technique, applied on the gradient boosting model. A high importance suggests that a Tag is precisely defined by a given feature, thus the feature is integral to the prediction of the Tag.


“Tech,” “SpeedTech,” “Offroad,” and “Fullspeed” generally have high feature importances, whereas “LOL” only minimally so. This is also reflected in Figure 4.2, wherein Tags with higher feature importances are more often correctly predicted. Basing ourselves on the model, we can conclude that “LOL” is the least well defined Tag. It is notable that the two Tags containing the word “technique” in their names are the most well-defined.
