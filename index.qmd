---
title: On the relation between Tags and Racing Paths of Trackmania Nations Forever Maps using Classification methods
subtitle: On the relation between Tags and Racing Paths of Trackmania Nations Forever Maps using Classification methods
...
\textbf{API}\quad{} An API, or Application-Program Interface, is an interface between a program and another. We primarily reference TMNF-X's API which allows communication with TMNF-X's database.

\quad

\textbf{Tag}\quad{} A Tag is a map's primary descriptor. While definitions for each Tag do exist for the newer Trackmania 2020 game, those definitions are not fully compatible with Trackmania Nations Forever which lacks such defitions.

\quad

\textbf{TMNF}\quad{} TMNF is the abbreviation for Trackmania Nations Forever, the arcade racing game on which this work is focused.

\quad

\textbf{TMNF-X}\quad{} TMNF-X is a website facilitating the act of sharing, between players, user-made Trackmania Nations Forever content, such as maps.

\quad

\textbf{Trackmania 2020}\quad{} The installment of the Trackmania series published in 2020.

\quad

\textbf{Replay}\quad{} A replay of the racing path taken by a player, stored as a `.Replay.GBX` file.
...
Plusieurs méthodes de classification de points de données existent, ayant tous leurs cas d’utilisation, avantages et désavantages différents. Pour le cas de la classification du cheminement dans une étiquette, il y a quatre exigences principales.
...
Afin de déterminer le modèle le plus représentatif, les modèles peuvent être comparés entre eux par moyen de leur précision, exactitude, sensibilité ou spécificité^[Certains indicateurs dépendent de distributions particulières des valeurs prédites. Par exemple, la spécificité est préférée lorsqu’il y a plus de faux positifs que de faux négatifs.] [@KeitaIntroClassification].
...
\centering
  ...
  \midrule
  Stunt & The goal is not speed, rather a stunt score. \\[10pt]
  Maze  & The goal is to find the best path and, having multiple valid \\
        & paths, the maps are often not ordered. \\[10pt]
  Laps  & The same map is completed more than once. \\[10pt]
  RPG   & The Tag is primarily composed of maps lasting hours straight, \\
        & with differences of a couple hours between each record. \\[10pt]
  Trial & The Tag is known for its extreme difficulty and longue \\
        & completion times (iBazztyB, et al, s. d.). \\
  \bottomrule
  \end{tabular}
\caption{Reasons for the exclusion of Tags from the sampleset.}
\label{Exclusions}
\end{table}
...
![Correlation matrices of the three statistical models: a dummy, a logistic regression, and a gradient booster (XGBoost).](rendered-figs/fig-model-stats.pdf){#fig-model-stats out-width=100% fig-align='center'}
...
#| fig-pos: 't'
#| echo: FALSE
#| fig-cap: "The relative distributions (relative to the minimums and maximums) of the values of each feature, per Tag. The red bars represent the standard deviation on each side of the average."
#| fig-width: 10
#| fig-height: 10
#| out-width: '100%'
#| fig-align: 'center'
...
ggplot(flat_data_long, aes(x=Tag, y=value)) + #  %>% group_by(Tag)
  ...
  # Anotations
  labs(
    title = "Relative Distributions of each Feature",
    x = "Tag",
    y = "Value"
  )
...
![The average impact of each feature on model predictions, that is, moy(|valeur SHAP|)](rendered-figs/fig-bar.pdf){#fig-importance-bar width=100% fig-align='center'}
...
En nous fixant sur la @fig-importance-bar, le « Tech » est défini par sa vitesse latérale (`AvgAbsSpeedSideward`), son montant de pilotage (`AvgAbsSteer`) et son déplacement horizontal (`AvgAbsDisplacementHorizontal`). Cela n’implique pas que les valeurs de ces trois variables doivent s’élever pour prédire « Tech », mais plutôt que leur proximité aux valeurs attendues est priorisée par le modèle. En reliant les données de la @fig-violin-facet, le « Tech » est mieux défini par une moyenne de vitesses absolues latérale aux alentours de `{r} filter(dfTab, Tag == "Tech" & measure == "AvgAbsSpeedSideward")[["mean"]]` km/h, un pilotage absolu moyen aux alentours de `{r} filter(dfTab, Tag == "Tech" & measure == "AvgAbsSteer")[["mean"]] * 100` % et une moyenne de déplacements horizontaux près de `{r} filter(dfTab, Tag == "Tech" & measure == "AvgAbsDisplacementHorizontal")[["mean"]]` m.
...
Le « Fullspeed » correspond à la première demie de nos hypothèses de la @sec-fullspeed à son sujet. Bien qu’il ait des rotations qui correspondent à nos hypothèses, il est caractérisé principalement par sa vitesse élevée. Sa vitesse moyenne vers l’avant (`AvgSpeedForward`) le caractérisant est centrée à `{r} filter(dfTab, Tag == "Fullspeed" & measure == "AvgSpeedForward")[["mean"]]` km/h et présente un écart type élevé de `{r} filter(dfTab, Tag == "Fullspeed" & measure == "AvgSpeedForward")[["sd"]]` km/h.
...
\textbf{Temps - Quantitative discrète}\quad{} Temps actuel de l’échantillon, ayant comme unité 100 millisecondes. (provenant de CGameGhost.Data.Sample.Time dans le fichier GBX)
...
<!-- ## Flattened Variables {#sec-annex-c} -->

\newcommand{\vardef}[3]{\textbf{#1} \textcolor{gray}{$\in$ #3}\quad #2}

\vardef{AvgAbsDisplacementHorizontal}{The average absolute horizontal displacement per unit of time.}{$[0,\infty[$}

\vardef{AvgAbsDisplacementY}{The average absolute vertical displacement per unit of time.}{$[0,\infty[$}

\vardef{AvgRPM}{The average of the motor's Revolutions per Minute (RPM).}{$[0,\infty[$}

\vardef{AvgSteerBias}{The average steering bias, expressed as a percentage. If the number of left key presses is equivalent to that of the right, the value is 0; they cancel out. If one of either directional keys is held throughout the replay, the value is 1.}{$[0,1]$}

\vardef{AvgAbsSteer}{The average steering amount, without distinction for direction, expressed as a percentage.}{$[0,1]$}

\vardef{AvgSpeedForward}{The average speed forward. Speed forward is positive while speed backward is negative.}{$]-\infty,\infty[$}

\vardef{AvgAbsSpeedForward}{The average speed, without distinction for direction.}{$[0,\infty[$}

\vardef{AvgSpeedSidewardBais}{The average lateral speed biais, without distinction for direction. If the leftward speeds cancel out the rightward speeds, the value is 0.}{$[0,\infty[$}

\vardef{AvgAbsSpeedSideward}{The average lateral speed, without distinction for direction.}{$[0,\infty[$}

\vardef{AvgSpeedSidewardOppSteer}{The average tendency of directional oposition between lateral speed and steering. The value tends towards 1 when both are of the same sign, and towards -1 when they are of differing signs.}{$[-1, 1]$}

\vardef{PercentPitch——Third}{The percentage of time units in which the car's pitch falls within a given third, that being the lower, middle or upper third.}{$[0, 1]$}

\vardef{PercentRoll——Third}{The percentage of time units in which the car's roll falls within a given third, that being the lower, middle or upper third.}{$[0, 1]$}

\vardef{PercentTurbo}{The percentage of time units in which the car is under the effect of a turbo.}{$[0, 1]$}

\begin{addmargin}[150pt]{0pt}
The relative distributions of the values of each feature, per Tag. The red bars represent the standard deviation on each side of the average.
\end{addmargin}
...
Trackmania Nations Forever — TMNF — is an arcade racing game that is often the subject of statistical analyses by players such as JstAnothrVirtuoso [@JstAnothr] and Yosh [@Yosh], as well as by projects such as Linesight [@Linesight].

Players can build their own maps, as well as publish them on platforms such as Trackmania Nations Forever Exchange — TMNF-X [@TMNFX]. Authors categorize their maps into a set of Tags, which act as descriptors of the general style and goal of a given map. It is worth noting that Tags are not given exact definitions — each author may interpret a Tag differently when assigning it —, and that more than one tag may be assigned to a given map. Tags may be “Fullspead,” “LOL,” among others.

Authors must use their subjective interpretation of the differences between each Tag to determine the most appropriate choice. It is, therefore, worth questioning **what primary characteristics of the car’s motion and path influence an author’s Tag choice**.



____________ Methodology __________________

**TMNF-X API ⇒ Sift ⇒ Parse & Flatten ⇒ Train/Test Split**

\begin{addmargin}[81pt]{0pt}
This study’s population consists of maps published on TMNF-X that have only one assigned Tag so as to allow multi-class classification, that do not require game modifications such as TMUnlimiter, and that are of type “Race.” The population size is, thus, approximately 600 000 maps (“Site Statistics,” n.d.). While multi-label classification would be preferable as it would allow maps with multiple assigned Tags to be classified, resulting complexities would exceed the scope of this project.

We, first, stratifiy by each map’s Tag, randomly choosing 1113 maps per stratum. Only eight of the thirteen possible Tags are used, as the others either go against the goal of this study or could not reasonably be analysed within given time constraints. The Tags selected for this study are:

{ \textcolor{gray}{Normal, Offroad, Fullspeed, LOL, Tech, SpeedTech, PressForward, Grass} }

Then, each map’s record — that being the Replay with the lowest finish time — is taken by means of TMNF-X’s API. Only maps with five or more submitted replays are considered. These two final conditions aim to reduce the number of partial-effort replays entering the sampleset, thus allowing better representation of the map’s true path.

After sampling, the replays are parsed so as to extract meta-data and to calculate other useful information, that is, to collect the dependent variable and the independent and confounding variables. All hierarchical data, such as the position at each time step, is then flattened into a format usable by classification models.


This data is then separated into training and test datasets. This prevents the models from overfitting to the training data. The use of external data, other than that which is used to train the models, hints to what the models truly understood. The goal is not to train models to memorize answers, instead to train models to follow patterns and generalize; interpolate and extrapolate.
\end{addmargin}
...
______________________ The Models ____________________________

We use a multi-class gradient boosting model — more specifically, XGBoost — as our primary model due to its numerous advantages for the particularities of this study. A logistic regression model is, then, used as a point of reference of simpler models. The last model, the dummy, serves as a minimal point of reference. This model’s exactitude equates to the mathematical expectation of randomly guessing the Tag, that is, $E(X) = \frac{1}{8} = 12.50\%$. Thus, a model that performs worse than the dummy model is less representative than guessing.

[Fig 1]

Figure 1 demonstrates the three models’ exactitudes by means of confusion matrices. Note that a perfect model would have no incorrectly assigned Tag and would, thus, form a perfect diagonal from the top left to the bottom right.

As expected, the dummy model, which always predicts the same Tag, is but minimally representative. The logistic regression model presents a better representativity, with a diagonal line beginning to form. The gradient boosting model surpasses both previous models, forming a nearly complete diagonal; it is well correlated. The only Tag that is often erroneously predicted is “LOL.”





____________________ Analysis ______________________

We determine each feature’s importance on the prediction of a Tag using the SHapley Additive exPlanations technique, applied on the gradient boosting model. A high importance suggests that a Tag is precisely defined by a given feature, thus the feature is integral to the prediction of the Tag.


“Tech,” “SpeedTech,” “Offroad,” and “Fullspeed” generally have high feature importances, whereas “LOL” only minimally so. This is also reflected in Figure 1, wherein Tags with higher feature importances are more often correctly predicted. Basing ourselves on the model, we can conclude that “LOL” is the least well defined Tag. It is notable that the two Tags containing the word “technique” in their names are the most well-defined.

“Tech” is best defined by its lateral speed (AvgAbsSpeedSideward), its steering amount (AvgAbsSteer) and its horizontal displacement (AvgAbsDisplacementHorizontal). This does not imply that these three variables must increase to predict “Tech,” rather that their proximity to expected values is prioritized by the model. Connecting data from Figure 2, “Tech” is best defined by direction-agnostic average lateral speeds around 4.34 km/h, average steering amounts near 71.93% and an average displacement nearing 5.87 m.


“SpeedTech” is primarily defined by its use of turbos (PercentTurbo) during around 3.42% of the driving. Also, its direction-agnostic steering amount is close to 63.48%. It is interesting to note that “Tech” and “SpeedTech,” which both have similar names, differentiate themselves by their focus on speed, as suggested by the use of the word “Speed.” “Tech” prefers turning, with higher steering amounts (AvgAbsSteer), whereas “SpeedTech” prefers turbos and higher, more varied, speeds (AvgSpeedForward, AvgAbsSpeedForward et AvgRPM).


“Offroad” is characterized by its average lateral speeds (AvgAbsSpeedSideward), and secondly by its steering and lateral speed biases (AvgSteerBias, AvgSpeedSidewardBias). Applying logic from the game’s physics, where there is but minimal traction on dirt, the car should slide or drift often. This is not the case as seen in Figure 2, all the while these features are attributed higher importances. The main characteristics of “Offroad” maps should, therefore, be further explored so as to better define it using the car’s motion.


“Fullspeed” is primarily characterized by high average speeds near 441.73 km/h, yet differing by large amounts as demonstrated by that feature’s standard deviation of 91.50 km/h. On average, the Tag covers a larger horizontal distance (AvgAbsDisplacementHorizontal), has higher revolutions per minute (AvgRPM) and forward speeds (AvgSpeedForward, AvgAbsSpeedForward), and is more often placed in the roll third that is parallel to walls (PercentRollMiddleThird). This supports our hypotheses as well as the definition from the newer Trackmania 2020 game, wherein “long flowing turns[,] loops, wallrides, and similar [are] common” (Eyebo [Rollins] 2024) and “high speeds [are] expected” (Eyebo [Rollins] 2024). Constant acceleration would raise the car’s RPM and forward speeds (AvgSpeedForward and AvgAbsSpeedForward). Given heightened speeds, long turns would push the car to traverse greater distances (AvgAbsDisplacementHorizontal).


“Grass” and “Normal” focus on the amount of steering (AvgAbsSteer) and the car’s average speed (AvgAbsSpeedForward). Neither have strong importances, and both have moderate speeds and steering amounts.


“PressForward” is primarily characterized by its direction-agnostic steering amount (AvgAbsSteer). While it may be evident to suppose that it is the only Tag in which players press only one key, numerous players perform various keypresses throughout replays. This is shown, in Figure 2, in its steering bias (AvgSteerBias) and amount of steering (AvgAbsSteer) which vary greatly. This suggests a contradiction between the Tag’s name and the primary driving method.

In addition, “PressForward” has the largest variances in general, as seen in Figure 2. This pattern suggests that this Tag is assigned to maps that differ greatly from one-another. This is in agreement with our hypothesis for which variance would be present due to extreme stunts. The presence of stunts can be inferred by large variations in horizontal and vertical displacements (AvgAbsDisplacementHorizontal, AvgAbsDisplacementY), in forward and lateral speeds (AvgSpeedForward, AvgAbsSpeedSideward), and in the car’s average orientation (PercentPitch——Third, PercentRoll——Third).


“LOL” is the only Tag for which the models did not discover notable features. Verifying with Figures 1 and 2, we can ascertain that it is the least well predicted. It is, therefore, likely that the “LOL” Tag is a mish-mash of driving styles.


_____________________ Conclusion ___________________________

“Tech” prefers turns. Its counterpart, “SpeedTech,” is similar, distinguishing itself by higher speeds. “Fullspeed” is characterized by generally high speeds. The characteristics of “Grass” and “Normal” maps are similar, those being their steering amount and speed, nonetheless, their expected values differ. “PressForward” is hard to define according to the model and data due to high variances and unexpected key presses; its name implies no steering, yet steering is present. “Offroad” and “LOL” require further research to conclude their primary characteristics.

Note that this is all in regards to the car’s motion and state. No data regarding track components, such as the presence of dirt or grass blocks, is collected.



<!-- ___________ Limitations and Biases, and Prevention Thereof _________________

Some sample data is missing from the hierarchical dataset as that storage method was introduced later in the sampling stage as a form of backup; the flattened dataset is the primary dataset. The data collected through the TMNF-X API was programmatically checked for malformations, missing data points, and collection errors (such as HTTP errors).


The use of a community-made parser for GBX files — that being GBX.NET2 —, not made by the original creators of the game, may result in slight differences in data compared to that which is actually contained within the files.

Some calculations during the data collection process can result in slightly altered data. One such calculation is the conversion of the car’s rotation from quaternions to Euler-space (pitch, yaw, roll). For another, the use of floating-point numbers may incur slight deviations due to rounding (Wagner, Bill, et al 2022).

A given car rotation can be represented by many different pitch-yaw-roll sets. We have given priority to pitch, then yaw, then roll — the default for GBX.NET2. This has resulted in PercentRollUpperThird always being zero, as another equivalent pitch-yaw-roll set is prioritized. -->


{{< pagebreak >}}


______________________ Note 1 __________________________

Note that no causal inference is possible since we have no control over the collected data. We cannot alter values and see the effects. Rather, due to the observational nature of this study, correlations are inferred.



____________________ Improvements / Future Work _______________________

A given car rotation can be represented by many different pitch-yaw-roll sets. We have given priority to pitch, then yaw, then roll — the default for GBX.NET2. This has resulted in PercentRollUpperThird always being zero, as another equivalent pitch-yaw-roll set is prioritized. This calculation should be revised.

Features focusing on the car’s rotation at a given moment minimally inform the selected Tag. In fact, these six features, PercentPitch——Third and PercentRoll——Third, are the six worst features in terms of feature importance. It is, therefore, worth re-evaluating the inclusion and implementation of these two features in the case of a future model. It may, also, be worth generating features en-masse, then itteratively discarding the worst ones to be left with the best features.

The current model can predict but one Tag at a time, which massively reduces maps that can be analysed. It would be much more useful if the model could perform multi-label classification, wherein maps with multiple Tags could be properly handled.

The model in its current form incorrectly predicts Tags roughly one in every five predictions. The addition of other track details, such as blocks present in the map, may increase the accuracy of a future model enough so as to be implemented as a Tag suggestion feature for authors submitting maps on TMNF-X*.

*TMNF-X does not tend to add superfluous features, meaning this would likely take the form of an external tool.
...
Eyebo [Rollins]. 2024. “Tags Explained.” Trackmania 2020 Exchange. https://trackmania.exchange/threads/700/tags-explained.

JstAnothrVirtuoso. 2024. “The WR Race - TMUF!” Youtube. https://www.youtube.com/watch?v=zn0h9mrIgpU.

Mania.Exchange. n.d. “TMNF-X.” https://tmnf.exchange/.

pb4git, et al. 2024. “I Trained an AI for 2 Years on Trackmania. It’s Breaking Records.” YouTube. https://www.youtube.com/watch?v=cUojVsCJ51I.

Pivoňka, Petr, et al. n.d. “GBX.NET.” Github. https://github.com/BigBang1112/gbx-net.

“Site Statistics.” n.d.Trackmania Nations Forever Exchange. https://tmnf.exchange/statistics.

“Yosh.” n.d.YouTube. https://www.youtube.com/@yoshtm.


\tiny
Works Cited: Eyebo [Rollins]. 2024. “Tags Explained.” Trackmania 2020 Exchange. https://trackmania.exchange/threads/700/tags-explained. ; JstAnothrVirtuoso. 2024. “The WR Race - TMUF!” Youtube. https://www.youtube.com/watch?v=zn0h9mrIgpU. ; Mania.Exchange. n.d. “TMNF-X.” https://tmnf.exchange/. ; pb4git, et al. 2024. “I Trained an AI for 2 Years on Trackmania. It’s Breaking Records.” YouTube. https://www.youtube.com/watch?v=cUojVsCJ51I. ; Pivoňka, Petr, et al. n.d. “GBX.NET.” Github. https://github.com/BigBang1112/gbx-net. ; “Site Statistics.” n.d.Trackmania Nations Forever Exchange. https://tmnf.exchange/statistics. ; “Yosh.” n.d.YouTube. https://www.youtube.com/\@yoshtm.