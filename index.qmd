...
_______________Introduction_______________
...
____________Methodology__________________

This study’s population consists of maps published on TMNF-X that have only one assigned Tag, that do not require game modifications such as TMUnlimiter, and that are of type “Race.” The population size is, thus, approximately 600 000 maps [@TMNFXStats].

*Table 2.1*
Five Tags, seen in Table 2.1, are excluded since they either go against the goal of this study, could not reasonably be analysed within given time constraints, or both simultaneously. Eight tags remain: {list of tags}.
...
______________________The Models____________________________

We use a multi-class gradient boosting model — more specifically, XGBoost — as our primary model due to its numerous advantages for the particularities of this study. A logistic regression model is, then, used as a point of reference of simpler models. The last model, the dummy, serves as a minimal point of reference. This model’s exactitude equates to the mathematical expectation of randomly guessing the Tag, that is, (𝐸(𝑋) = 1/8 = 12,50% ). Thus, a model that performs worse than the dummy model is less representative than guessing.

[Fig 4.2]

Figure 4.2 demonstrates the three models’ exactitudes by means of confusion matrices. Note that a perfect model would have no incorrectly assigned Tag and would, thus, form a perfect diagonal from the top left to the bottom right.

As expected, the dummy model, which always predicts the same Tag, is but minimally representative. The logistic regression model presents a better representativity, with a diagonal line starting to form. The gradient boosting model surpasses both previous models, forming a nearly complete diagonal, well correlated. The only Tag that is often erroneously predicted is “LOL.”




____________________Analysis______________________
...
____________________Improvements / Future Work_______________________
