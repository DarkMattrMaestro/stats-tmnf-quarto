...
“Tech,” “SpeedTech,” “Offroad,” and “Fullspeed” generally have high feature importances, whereas “LOL” only minimally so. This is also reflected in Figure 2, wherein Tags with higher feature importances are more often correctly predicted. Basing ourselves on the model, we can conclude that “LOL” is the least well defined Tag. It is notable that the two Tags containing the word “technique” in their names are the most well-defined.

“Tech” is best defined by its lateral speed (AvgAbsSpeedSideward), its steering amount (AvgAbsSteer) and its horizontal displacement (AvgAbsDisplacementHorizontal). This does not imply that these three variables must increase to predict “Tech,” rather that their proximity to expected values is prioritized by the model. Connecting data from Figure 4.1, “Tech” is best defined by direction-agnostic average lateral speeds around 4.34 km/h, average steering amounts near 71.93 % and an average displacement nearing 5.87 m.


“SpeedTech” is primarily defined by its use of turbos (PercentTurbo) during around 3.42% of the driving. Also, its direction-agnostic steering amount is close to 71.93%. It is interesting to note that “Tech” and “SpeedTech,” which both have similar names, differentiate themselves by their focus on speed, as suggested by the use of the word “Speed.” “Tech” prefers turning, with higher steering amounts (AvgAbsSteer), whereas “SpeedTech” prefers turbos and higher, more varied, speeds (AvgSpeedForward, AvgAbsSpeedForward et AvgRPM).


“Offroad” is characterized by its average lateral speeds (AvgAbsSpeedSideward), and secondly by its steering and lateral speed biases (AvgSteerBias, AvgSpeedSidewardBias). Applying logic from the game’s physics, where there is but minimal traction on dirt, the car should slide or drift often. This is not the case as seen in Figure 4.1, all the while these features are attributed higher importances. The main characteristics of “Offroad” maps should, therefore, be further explored so as to better define it using the car’s motion.
...
“PressForward” is primarily characterized by its direction-agnostic steering amount (AvgAbsSteer). While it may be evident to suppose that it is the only Tag in which players press only one key, numerous players perform various keypresses throughout replays. This is shown, in Figure 4.1, in its steering bias (AvgSteerBias) and amount of steering (AvgAbsSteer) which vary greatly. This suggests a contradiction between the Tag’s name and the primary driving method.

In addition, “PressForward” has the largest variances in general, as seen in Figure 4.1. This pattern suggests that this Tag is assigned to maps that differ greatly from one-another. This is in agreement with our hypothesis for which variance would be present due to extreme stunts. The presence of stunts can be inferred by large variations in horizontal and vertical displacements (AvgAbsDisplacementHorizontal, AvgAbsDisplacementY), in forward and lateral speeds (AvgSpeedForward, AvgAbsSpeedSideward), and in the car’s average orientation (PercentPitch——Third, PercentRoll——Third).


“LOL” is the only Tag for which the models did not discover notable features. Verifying with FIgure 4.2 and Table 4.1, we can ascertain that it is the least well predicted. It is, therefore, likely that the “LOL” Tag is a mish-mash of driving styles.
